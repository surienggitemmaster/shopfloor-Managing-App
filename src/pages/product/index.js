...
const ProductPage = () => {
  ...
  return (
    <>
      ...
      {(files && setFolderId) ? (<PDFViewer fileID={selectfileId} />) : (<Loader />)}
      ...
      <aside ref={refForSidebar} id="default-sidebar" className={`fixed top-0 left-0 z-40 w-64 h-screen transition-transform ${sidebarOpenForMobile ? "translate-x-0" : "-translate-x-full sm:translate-x-0"} `} aria-label="Sidebar">
        <div className="h-full px-3 py-4 text-xs sm:text-base overflow-y-auto bg-gray-100 dark:bg-gray-800 border-r border-gray-300">
