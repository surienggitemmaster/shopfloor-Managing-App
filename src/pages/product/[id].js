  ...
  return (
    <>
      <aside ref={refForSidebar} id="default-sidebar" className={`${inter.className} fixed top-0 left-0 z-40 w-64 h-screen transition-transform ${sidebarOpenForMobile ? "translate-x-0" : "-translate-x-full sm:translate-x-0"} `} aria-label="Sidebar">
        <div className="h-full px-3 py-4 text-xs sm:text-base overflow-y-auto bg-gray-200 dark:bg-gray-800 border-r dark:border-gray-500 border-gray-300">
          <div onClick={() => setSidebarOpenForMobile(false)} className='sm:hidden flex items-center justify-end text-sm space-x-2'>
            <span>Collapse</span>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-5">
              <path fillRule="evenodd" d="M16.5 3.75a1.5 1.5 0 0 1 1.5 1.5v13.5a1.5 1.5 0 0 1-1.5 1.5h-6a1.5 1.5 0 0 1-1.5-1.5V15a.75.75 0 0 0-1.5 0v3.75a3 3 0 0 0 3 3h6a3 3 0 0 0 3-3V5.25a3 3 0 0 0-3-3h-6a3 3 0 0 0-3 3V9A.75.75 0 1 0 9 9V5.25a1.5 1.5 0 0 1 1.5-1.5h6ZM5.78 8.47a.75.75 0 0 0-1.06 0l-3 3a.75.75 0 0 0 0 1.06l3 3a.75.75 0 0 0 1.06-1.06l-1.72-1.72H15a.75.75 0 0 0 0-1.5H4.06l1.72-1.72a.75.75 0 0 0 0-1.06Z" clipRule="evenodd" />
            </svg>
          </div>
          <div className='border bg-white border-gray-300 rounded-md p-2 mt-2 sm:mt-0'>
          ...
          <ul className="space-y-2 font-medium mt-2 text-sm">
            ...
            <li onClick={() => handleFileId({ key: 'drawing', value: files?.DRAWING })} >
              <a className={`flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-white ${(activeTab === 'drawing') && 'bg-white dark:bg-gray-700'} dark:hover:bg-gray-700 group cursor-pointer`}>
            ...
            <li onClick={() => handleFileId({ key: 'process', value: files?.PROCESS })}>
              <a className={`flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-white ${(activeTab === 'process') && 'bg-white dark:bg-gray-700'} dark:hover:bg-gray-700 group cursor-pointer`}>
            ...
            <li onClick={() => handleFileId({ key: 'sellerDetails', value: files?.SELLER_DETAILS })}>
              <a className={`flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-white ${(activeTab === 'sellerDetails') && 'bg-white dark:bg-gray-700'} dark:hover:bg-gray-700 group cursor-pointer`}>
            ...
            <li onClick={() => handleFileId({ key: 'purchaseDetails', value: files?.PURCHASE_DETAILS })}>
              <a className={`flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-white ${(activeTab === 'purchaseDetails') && 'bg-white dark:bg-gray-700'} dark:hover:bg-gray-700 group cursor-pointer`}>
            ...
            <li onClick={() => handleFileId({ key: 'inspectionDetails', value: files?.INSPECTION_DETAILS })}>
              <a className={`flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-white ${(activeTab === 'inspectionDetails') && 'bg-white dark:bg-gray-700'} dark:hover:bg-gray-700 group cursor-pointer`}>
            ...
            <li onClick={() => handleFileId({ key: 'customerComplaint', value: files?.CUSTOMER_COMPLAINT })}>
              <a className={`flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-white ${(activeTab === 'customerComplaint') && 'bg-white dark:bg-gray-700'} dark:hover:bg-gray-700 group cursor-pointer`}>
            ...
            {otherFiles.length > 0 && (
              <li className={`rounded-lg ${expandOtherFiles && 'dark:bg-gray-700 bg-white'}`} onClick={() => setExpandOtherFiles(!expandOtherFiles)}>
                <a className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-white dark:hover:bg-gray-700 group cursor-pointer">
                ...
                {expandOtherFiles && (
                  <div className=''>
                    <ul className='space-y-2 font-medium p-2 text-sm'>
                      {Object.keys(files)
                        .filter(fileName => !fieldsToExclude
                          .includes(fileName))
                        .map(fileName => (
                          <li onClick={(e) => {
                            e.stopPropagation();
                            handleFileId({ key: fileName, value: files[fileName] })
                          }} key={fileName} className='pl-8 text-gray-900 rounded-lg dark:text-white cursor-pointer'>{fileName}</li>
                        ))}
                    </ul>
                  </div>
                )}
      ...
      {/* main screen */}
      <div className={`${inter.className} relative sm:ml-64 flex flex-col h-screen`}>
        <div className='flex text-xs sm:text-sm text-gray-900 bg-gray-200 dark:bg-gray-800 border-b dark:border-gray-500 dark:text-white'>
          ...
          <div className='p-2'>
            ...
            <div className='flex space-x-2'>
              <h4 className='shrink-0'>Product Name</h4>
              <p className=''>{name}</p>
            </div>
          </div>
